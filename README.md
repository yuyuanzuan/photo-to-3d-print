...
服务默认监听 `0.0.0.0`，启动日志会直接把可用地址打出来：

```
流水线 UI 已启动
  本机:    http://127.0.0.1:8765
  局域网:  http://192.168.100.113:8765
```

同一 WiFi 下手机、平板、别的电脑直接打开即可，方便拍完照直接上传。

⚠️ **服务本身没有登录鉴权。** 局域网内任何人打开这个地址，都能用你的
API 额度、看到并下载所有输出文件。只在自有可信网络里开。

想只允许本机访问，改 `config.json`：

```json
{ "listen_host": "127.0.0.1" }
```

或设环境变量 `PIPELINE_HOST=127.0.0.1`。
